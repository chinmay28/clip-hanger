...
/* The badge thrown up full screen for a beat. Everything here is on one clock:
   ...
   Portalled to the body so "full screen" stays true wherever the mark is
   tapped from — a transformed ancestor would otherwise become what this
   measures itself against. */
function DevFlash({ onDismiss }) {
  return createPortal(
    <div
      onClick={onDismiss}
      className="clip-dev-veil"
      style={{
        position: 'fixed',
        inset: 0,
        ...
        animation: 'clip-dev-veil 3s ease-in-out forwards',
      }}
    >
      <div
        className="clip-dev-lockup"
        style={{
          display: 'flex',
          ...
          // apart mid-animation.
          animation: 'clip-dev-badge 3s cubic-bezier(0.22, 1, 0.36, 1) forwards',
        }}
      >
        <img
